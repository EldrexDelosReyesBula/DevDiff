...
describe("DevDiff v1.7.0 — Community Growth & Feedback Engine", () => {
  ...
  it("verifies GitHub issue templates exist and contain required headers", () => {
    ...
    const featureReq = fs.readFileSync(path.join(templatesDir, "feature_request.md"), "utf-8");
    expect(featureReq).toContain("What Would You Like DevDiff to Do?");

    const genFeedback = fs.readFileSync(path.join(templatesDir, "general_feedback.md"), "utf-8");
    expect(genFeedback).toContain("What's on Your Mind?");

    const bugReport = fs.readFileSync(path.join(templatesDir, "bug_report.md"), "utf-8");
