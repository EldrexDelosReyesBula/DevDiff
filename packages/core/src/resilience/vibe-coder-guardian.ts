...
export class VibeCoderGuardian {
  ...
  constructor() {
    ...
    this.sessionPath = path.resolve(process.cwd(), '.devdiff/vibe-session.json');
  }
  ...
  async saveSession(): Promise<void> {
    ...
    await fs.writeFile(this.sessionPath, JSON.stringify(this.session, null, 2), 'utf-8');
  }
  ...
  private async captureSnapshot(files: string[]): Promise<{ files: [string, string][] }> {
  ...
  async handleFailure(failure: {
    ...
    const transparency = {
      ...
      whatHappensNext: failure.attempt < 3 
  ...
  async restoreCheckpoint(checkpointId: string): Promise<void> {
    let checkpoint = this.session.checkpoints.find(c => c.id === checkpointId);
    
    if (!checkpoint) {
      // Try to load checkpoint directly from disk if not loaded in session
      try {
        const fileContent = await fs.readFile(path.join(this.checkpointDir, `${checkpointId}.json`), 'utf-8');
