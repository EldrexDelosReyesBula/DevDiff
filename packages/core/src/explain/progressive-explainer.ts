export type ExplanationLevel = "beginner" | "student" | "developer" | "senior" | "architect";
...
export class ProgressiveExplainer {
  private static readonly LEVEL_CONFIGS: Record<ExplanationLevel, LevelConfig> = {
    beginner: {
      ...
      audience: "Someone learning to code. Explain everything. No assumption of prior knowledge.",
    },
    student: {
      name: "Student",
      icon: "📚",
      ...
      audience: "Knows programming basics. Learning design patterns, architecture, and best practices.",
    },
    developer: {
      name: "Developer",
      icon: "💻",
      ...
      audience: "Professional developer. Needs to understand what changed, why, and impact.",
    },
    senior: {
      name: "Senior",
      icon: "🧠",
      ...
      audience: "Senior developer. Focus on performance implications, memory impact, breaking changes, and migration paths.",
    },
    architect: {
      name: "Architect",
      icon: "🏗️",
      ...
      audience: "Software architect. Focus on system-wide impact, design patterns, technical debt, compliance, and long-term strategy.",
    },
  };
  ...
  static async explain(params: {
    ...
    const config = this.LEVEL_CONFIGS[params.level] || this.LEVEL_CONFIGS.developer;
   ...
   */
  static autoDetectLevel(code: string, projectContext: ProjectContext = {}): ExplanationLevel {
    if (!code) return "beginner";

    const lines = code.split("\n").length;
    const hasComplexPatterns = /class\s+\w+.*extends|implements|abstract|generic|decorator|middleware|async\s+function/.test(code);
    const hasPerformanceCode = /performance|optimize|cache|memoize|debounce|throttle|worker|pool/.test(code);
    const hasSecurityCode = /auth|encrypt|hash|token|session|password|secret|key/.test(code);
  ...
  private static async generateSummary(params: { code: string; filePath: string }, config: LevelConfig): Promise<string> {
  ...
  private static async generateLineByLine(params: { code: string }, config: LevelConfig): Promise<string> {
    const lines = params.code.split("\n").slice(0, 15);
    return lines
      .map((line, idx) => `- **Line ${idx + 1}:** \`${line.trim()}\` — ${config.includeAnalogies ? "Acts like a step-by-step instruction." : "Executes code block."}`)
      .join("\n");
  }

  private static async generateConcepts(params: { code: string }, config: LevelConfig): Promise<string> {
    const concepts: string[] = [];
    if (/function|=>/.test(params.code)) concepts.push("Functions & Execution Blocks");
    if (/class/.test(params.code)) concepts.push("Object-Oriented Encapsulation");
    if (/async|await|Promise/.test(params.code)) concepts.push("Asynchronous Control Flow");
  ...
  private static async generateArchitectureImpact(params: { filePath: string }, config: LevelConfig): Promise<string> {
  ...
  private static async generatePerformanceAnalysis(params: { code: string }, config: LevelConfig): Promise<string> {
    return [
      "### Performance Profile",
  ...
  private static async generateBreakingChanges(params: { code: string }, config: LevelConfig): Promise<string> {
    return [
  ...
  private static async generateBestPractices(params: { code: string }, config: LevelConfig): Promise<string> {
    return [
  ...
  private static async generateTradeoffs(params: { code: string }, config: LevelConfig): Promise<string> {
    return [
  ...
  private static async generateTryItYourself(params: { filePath: string }, config: LevelConfig): Promise<string> {
  ...
  private static async generateStrategicRecommendations(params: { filePath: string }, config: LevelConfig): Promise<string> {
    return [
  ...
  private static async generateKeyTakeaways(params: { code: string }, config: LevelConfig): Promise<string[]> {
    return [
