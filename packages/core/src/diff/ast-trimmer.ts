...
export function trimAST(filename: string, code: string, changedLines: number[]): string {
  ...
  try {
      ...
      if (ts.isClassDeclaration(node)) {
        if (!nodeOverlapsChange(node)) {
          ...
          if (firstToken && lastToken) {
            ...
            const nameEnd = node.name ? node.name.getEnd() : node.getStart(sourceFile) + 5
            spansToReplace.push({
              start: nameEnd,
              end: end,
    ...
    console.warn(`AST trimming failed for ${filename}, falling back to line-based trimming:`, error)
    return trimLineBased(code, changedLines)
  }
}
...
function trimLineBased(code: string, changedLines: number[], contextLines = 5): string {
