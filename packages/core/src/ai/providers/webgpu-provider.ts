...
export class WebGPUProvider implements AIProvider {
  ...
  async initialize(config?: {
    ...
    await this.loadModel(
      config || { model: "llama3.2-3b-q4", quantization: "int4" },
    );
  ...
  async generate(prompt: string): Promise<AsyncGenerator<string>> {
    ...
    console.log(
      `⚡ GPU inference: ${elapsed.toFixed(1)}ms (${(tokens.length / (elapsed / 1000)).toFixed(0)} tok/s)`,
    );
  ...
  private tokenize(prompt: string): number[] {
    return Array.from({ length: prompt.length }, (_, i) =>
      prompt.charCodeAt(i),
    );
  }
