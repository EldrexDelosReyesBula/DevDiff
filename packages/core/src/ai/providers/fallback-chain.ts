...
export class LocalInferenceChain implements AIProvider {
  ...
  async initialize(): Promise<void> {
    const backends = [
      { name: 'WebGPU', check: () => this.checkWebGPU(), create: () => new WebGPUProvider() },
      { name: 'WASM', check: () => this.checkWASM(), create: () => new WASMProvider() },
      { name: 'CPU', check: () => this.checkCPU(), create: () => new NativeCPUProvider() },
    ]
    ...
    if (this.providers.length === 0) {
      console.log('📥 No backend available. Download a model with: ollama pull llama3.2:3b')
  ...
  async generateExplanation(diffText: string, modelName: string): Promise<AIExplanationResult> {
    if (this.providers.length === 0) {
      throw new Error('No inference backend available. Install Ollama or enable WebGPU.');
    }
  ...
  async generate(prompt: string): Promise<AsyncGenerator<string>> {
    ...
    if ('generate' in firstProvider && typeof (firstProvider as any).generate === 'function') {
