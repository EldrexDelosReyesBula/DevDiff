...
export class ProgressiveChunking {
  ...
  static async processWithFallback(
    ...
    processFn: (chunk: DiffChunk, timeoutMs: number) => Promise<AIExplanationResult>,
    ...
    while (currentSizeIndex < this.CHUNK_SIZES.length) {
      ...
      try {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          const timeoutMs = DynamicTimeout.calculate({
          ...
          try {
            ...
            console.log(`   ⚠️ Chunk ${i + 1} failed: ${(chunkError as Error).message}`);
            results.push({
              ...
              files: chunk.files.map(f => ({
                ...
                explanation: "Analysis unavailable due to timeout or generation error."
              })),
              relatedIssues: []
            });
          }
        ...
        console.log(`   ⚠️ ${sizeConfig.label} chunks failed. Trying smaller chunks...`);
        currentSizeIndex++;
        
        if (currentSizeIndex >= this.CHUNK_SIZES.length) {
          // All strategies exhausted — use template
          throw new Error('All chunking strategies exhausted. Falling back to template mode.');
        }
      }
    }
  ...
  private static chunkBySize(diff: ParseResult, maxFiles: number): DiffChunk[] {
    ...
    for (let i = 0; i < sorted.length; i += maxFiles) {
      ...
      const estimatedTokens = chunkFiles.reduce((sum, f) => {
      ...
      chunks.push({
        ...
        files: chunkFiles,
        label: chunkFiles.map(f => path.basename(f.path || f.newPath || f.oldPath || 'unknown')).join(', '),
