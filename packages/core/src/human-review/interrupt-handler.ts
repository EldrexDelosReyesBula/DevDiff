...
export interface ReviewRequest {
  id: string;
  type: 'changelog' | 'security_finding' | 'architecture_decision' | 'compliance_issue';
...
export class HumanReviewSystem {
  ...
  async requestReview(request: ReviewRequest): Promise<ReviewDecision> {
    ...
    if (request.aiOutput.split('\n').length > 5) {
      console.log(`│  ... ${request.aiOutput.split('\n').length - 5} more lines                 │`);
    }
    console.log('├─────────────────────────────────────────────┤');
    ...
    console.log('  [R] Reject  — Discard AI output, retry with different approach');
      ...
      rl.question('  Your decision [A/R/M/D]: ', (answer) => {
        ...
        switch (answer.toUpperCase()) {
          ...
          case 'M':
            ...
            break;
          case 'D':
            rl.question('  Delegate to which agent? [architect/security/performance/docs]: ', (agent) => {
              resolve({
                action: 'delegate',
                reviewer: 'human',
  ...
  private autoHandle(request: ReviewRequest): ReviewDecision {
    if (request.priority === 'critical') {
      return {
        action: 'reject',
        reviewer: 'auto',
        reason: 'Critical priority requires human review — auto-rejected for safety'
      };
    }
  ...
  getReviewStats(): ReviewStats {
    ...
    const approved = this.reviewHistory.filter(r => r.action === 'approve').length;
    const rejected = this.reviewHistory.filter(r => r.action === 'reject').length;
    const modified = this.reviewHistory.filter(r => r.action === 'modify').length;
    const delegated = this.reviewHistory.filter(r => r.action === 'delegate').length;
    const humanReviewed = this.reviewHistory.filter(r => r.reviewer === 'human').length;
