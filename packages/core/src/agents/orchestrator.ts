...
export class MultiAgentOrchestrator {
  ...
  async analyze(diff: ProcessedDiff): Promise<CollaborativeAnalysis> {
    ...
    console.log(
      `✅ Analysis complete: ${this.agents.size} agents collaborated`,
    );
  ...
  private async generateDiscussionPoint(
    analysis: AgentAnalysis,
    peerAnalysis: AgentAnalysis,
  ): Promise<string> {
  ...
  private async buildConsensus(
    ...
    return {
      findings: Array.from(findings.entries()).map(([finding, supports]) => ({
        ...
        status:
          supports.length / this.agents.size > 0.5 ? "consensus" : "disputed",
      })),
    };
  }
  ...
  private async synthesizeReport(
    ...
    const reportText = [
      ...
      "",
      "Consensus Findings:",
      ...consensus.findings.map(
        (f) =>
          `- [${f.status.toUpperCase()}] ${f.finding} (Score: ${f.consensusScore})`,
      ),
      "",
      "Agent Details:",
      ...analyses.map(
        (a) => `- Agent ${a.agentId} findings: ${a.findings.join("; ")}`,
      ),
    ].join("\n");
