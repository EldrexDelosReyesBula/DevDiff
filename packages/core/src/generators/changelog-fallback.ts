...
export class TemplateFallbackGenerator {
  ...
  static generate(diff: ParseResult, context: DeepContext | null): string {
    ...
    lines.push("");
    lines.push(`> ⚠️ **Template-generated** — AI was unavailable for this analysis.`);
    lines.push(`> This changelog is based on deterministic git analysis.`);
    lines.push(`> For AI-powered explanations, ensure your AI provider is running and try again:`);
    lines.push(`> \`\`\`bash`);
    ...
    lines.push(`| Lines Added | +${(diff.totalAdditions || 0).toLocaleString()} |`);
    lines.push(`| Lines Removed | -${(diff.totalDeletions || 0).toLocaleString()} |`);
    
    const commitCount = (diff as any).commitCount !== undefined ? (diff as any).commitCount : (context?.git?.totalCommits || 1);
    lines.push(`| Commits | ${commitCount} |`);
    
    const branchesList = (diff as any).branches || (context?.git?.activeBranch ? [context.git.activeBranch] : ["current"]);
    ...
    if (byStatus.modified.length > 0) {
      ...
      for (const file of byStatus.modified.slice(0, 20)) {
        const filePath = file.path || file.newPath || file.oldPath || "unknown";
        lines.push(`- \`${filePath}\` (+${file.additions || 0} -${file.deletions || 0})`);
      }
    ...
    if (byDir.size > 1) {
      ...
      lines.push("");
      for (const [dir, count] of Array.from(byDir.entries()).sort((a, b) => b[1] - a[1])) {
    ...
    lines.push(`3. **MVP Queue**: Full analysis saved for background processing`);
    lines.push(`4. **Manual Review**: Review this template changelog`);
    lines.push("");
    lines.push("---");
    lines.push("*Generated by DevDiff v1.0.5 — Template Mode (AI unavailable)*");
  ...
  private static groupByDirectory(files: ParsedFileDiff[]): Map<string, number> {
  ...
  private static detectPotentialIssues(diff: ParseResult): string[] {
    ...
    for (const deleted of deletedFiles) {
      ...
      if (isReferenced) {
        issues.push(`⚠️ Deleted \`${deleted.oldPath}\` may still be referenced in modified files`);
      }
    }
    
    // Check for large files
    const largeFiles = diff.files.filter(f => ((f.additions || 0) + (f.deletions || 0)) > 1000);
    if (largeFiles.length > 0) {
      issues.push(`📏 ${largeFiles.length} file(s) with >1000 line changes — consider reviewing`);
    }
    ...
    if (configChanges.length > 0) {
      issues.push(`🔒 ${configChanges.length} configuration file(s) changed — verify no secrets exposed`);
    }
