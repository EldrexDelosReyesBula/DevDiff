...
export async function generateChangelog(options: GenerateOptions): Promise<GenerateResult> {
  ...
  for (const fileDiff of parserResult.files) {
    ...
    const isIgnored = config.exclude.some(pattern => {
      ...
      return (newPath && newPath.includes(cleanPattern)) || (oldPath && oldPath.includes(cleanPattern))
    })
    ...
    if (fileContent && newPath) {
      ...
      contextCode = fileDiff.hunks.map(h => h.lines.map(l => l.content).join('\n')).join('\n')
    }
  ...
  const explanation = await router.getExplanation(diffContext, { dryRun: options.dryRun })
...
function formatOutput(result: AIExplanationResult, format: 'markdown' | 'json' | 'html'): string {
  switch (format) {
    case 'json':
