...
describe("MermaidSanitizer", () => {
  describe("toNodeId()", () => {
    it("converts hyphenated names", () => {
      expect(MermaidSanitizer.toNodeId("my-module-name")).toBe(
        "my_module_name",
      );
    });

    it("converts paths to valid IDs", () => {
      expect(MermaidSanitizer.toNodeId("src/components/Button.tsx")).toBe(
        "src_components_Button_tsx",
      );
    });

    it("handles special characters", () => {
      expect(MermaidSanitizer.toNodeId("user@domain.com")).toBe(
        "user_domain_com",
      );
    });
    ...
    it("handles unicode", () => {
      expect(MermaidSanitizer.toNodeId("café-component")).toMatch(
        /^[a-zA-Z0-9_]+$/,
      );
    });
  });
