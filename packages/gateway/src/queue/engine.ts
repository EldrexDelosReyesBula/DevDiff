...
export class TieredQueueEngine extends EventEmitter {
  ...
  private activeJobs: Map<string, { batch: Batch; status: string; result?: any }> = new Map()
  ...
  private findMergeableBatch(repoKey: string, event: DevDiffEvent): Batch | null {
    const queue = this.repoQueues.get(repoKey)
  ...
  private async processBatch(batch: Batch): Promise<any> {
    ...
    return new Promise(resolve => setTimeout(() => resolve({ success: true, processedEvents: batch.events.length }), 100))
  }
