...
export class DevDiffGateway {
  ...
  private async handleWebhookEvent(
    parsed: { event: string; payload: any } | null,
  ...
  async start() {
                ...
                tools: [
                  {
                    ...
                    description:
                      "Analyzes code changes and returns persona-aware explanation.",
                    inputSchema: {
                      type: "object",
                      properties: {
                        range: {
                  ...
                  content: [
                    {
                      type: "text",
                      text:
                        resultContext.outputs?.["markdown"] ||
    ...
    await Promise.all([
      ...
      new Promise<void>((resolve) =>
        this.openclawServer.listen(openclawPort, resolve),
      ),
  ...
  async stop() {
    ...
    await Promise.all([
      ...
      new Promise<void>((resolve) =>
        this.openclawServer.close(() => resolve()),
      ),
