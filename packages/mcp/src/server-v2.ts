  ...
  tools: [
    {
      ...
      description: 'Analyze git changes and generate an intelligent changelog. Use when the user asks about recent changes, what changed today, or wants a changelog.',
      inputSchema: {
        type: 'object',
        properties: {
          since: {
            type: 'string',
            description: 'Time range or git revision. Examples: "today", "24h", "1 week", "HEAD~10..HEAD", "last commit"'
          },
          persona: {
            type: 'string',
            enum: ['developer', 'ceo', 'educator', 'pm', 'compliance', 'journalist', 'data-analyst', 'robot'],
            description: 'Output style. Use "ceo" for executives, "developer" for code reviews, "pm" for product updates.'
          },
          format: {
            type: 'string',
    ...
    },
    {
      ...
      description: 'Perform security-focused code review. Use when user asks about security implications, vulnerabilities, or compliance.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Explain a specific file change or commit in detail. Use when user asks "why was this file changed?" or "explain this commit".',
      inputSchema: {
        type: 'object',
        properties: {
          file_path: { type: 'string', description: 'Path to the file' },
          commit_sha: { type: 'string', description: 'Specific commit hash (optional)' }
        },
        required: ['file_path']
      }
    },
    {
      ...
      description: 'Check changes against a compliance framework. Use when user asks about GDPR, HIPAA, SOC2 compliance.',
      inputSchema: {
        type: 'object',
        properties: {
          framework: {
            type: 'string',
            enum: ['gdpr', 'hipaa', 'soc2', 'fedramp', 'iso27001', 'pipeda', 'lgpd', 'pdpa']
          },
          since: { type: 'string' }
        },
    ...
    },
    {
      ...
      description: 'Generate architecture or dependency diagram from recent changes. Use when user asks to visualize changes.',
      inputSchema: {
        type: 'object',
        properties: {
          type: {
    ...
    },
    {
      ...
      description: 'Get project overview — what this codebase does, its architecture, key modules. Use when user asks "what is this project?" or needs context.',
      inputSchema: {
        type: 'object',
        properties: {}
      }
    },
    {
      ...
      description: 'Find changes related to a specific function, class, or module. Use when user asks "what else changed with X?"',
      inputSchema: {
        type: 'object',
        properties: {
          identifier: { type: 'string', description: 'Function name, class name, or file path' },
    ...
    {
      name: 'devdiff_status',
      description: 'Get current DevDiff status — active session, AI provider, recent changes count. Use when user asks "is DevDiff running?"',
      inputSchema: {
        type: 'object',
        properties: {}
      }
...
server.setRequestHandler(ListPromptsRequestSchema, async () => ({
  prompts: [
    ...
    {
      name: 'pr-review',
      description: 'Generate a pull request review with security and architecture analysis',
      arguments: [
    ...
    },
    {
      ...
      description: 'Generate a new developer onboarding summary of the codebase',
      arguments: []
    }
  ]
}))
    ...
    case 'devdiff_analyze': {
      ...
      return {
        content: [
          {
            type: 'text',
            text: typeof result === "string" ? result : (result.summary || JSON.stringify(result, null, 2))
          }
        ]
      }
    }
...
function formatComplianceResults(result: any): string {
  ...
  if (result.findings && result.findings.length > 0) {
    ...
    for (const f of result.findings) {
      const statusIcon = f.status === 'passed' ? '✅' : f.status === 'failed' ? '❌' : '⚠️';
...
export async function startMcpServer(options: { transport: 'stdio' | 'http'; port?: number }) {
