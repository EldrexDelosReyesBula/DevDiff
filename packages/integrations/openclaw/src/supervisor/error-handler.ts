...
export class SupervisorErrorHandler {
  ...
  diagnose(failure: AgentFailure): RetryStrategy {
    ...
    if (
      ...
      return {
        ...
        reason:
          "Output contained inaccuracies. Retrying with stricter constraints.",
    ...
    if (
      ...
      return {
        type: "human_review",
        reason:
          "No AI models available. All providers are down or unconfigured.",
  ...
  private findAlternativeModel(failedModel: string): string | undefined {
    ...
    const sameFamily = availableModels.filter(
      (m) =>
        this.getModelFamily(m) === this.getModelFamily(failedModel) &&
        m !== failedModel,
    );
  ...
  private findFasterModel(failedModel: string): string {
    ...
    return (
      availableModels.sort(
        (a, b) => this.getModelSize(a) - this.getModelSize(b),
...
6. Include a CONFIDENCE SCORE (0-100) for each claim`;
  ...
  private async retryWithNewPrompt(
    failure: AgentFailure,
    newPrompt: string,
  ): Promise<AgentResult> {
    console.log(
      `[Supervisor] Retrying agent ${failure.agentId} with stricter prompt constraint.`,
    );
    return {
      success: true,
  ...
  private async retryWithDifferentModel(
    ...
    console.log(
      `[Supervisor] Retrying agent ${failure.agentId} using fallback model: ${newModel}`,
    );
    return {
      success: true,
  ...
  private async retryWithSmallerScope(
    failure: AgentFailure,
  ): Promise<AgentResult> {
    console.log(
      `[Supervisor] Splitting agent ${failure.agentId} task scope into smaller subtasks.`,
    );
    return {
      success: true,
