...
} from "./autocomplete/config-autocomplete";
...
async function resolveModuleTypeWarning(): Promise<void> {
  ...
  try {
    ...
    if (!pkg.type) {
      ...
      if (hasESMImports) {
        console.log(
          '💡 Tip: Your project uses ES modules but package.json has no "type" field.',
        );
...
async function executeCommand(fullPath: string, args: any[]) {
  ...
  switch (parent) {
    ...
    case "plugin":
      ...
      console.log(
        "   CLI plugin management commands will be released in v1.0.6.",
      );
      return;
    default:
  ...
  for (const cmd of commands) {
    ...
    command.action(async (...actionArgs) => {
      try {
        ...
        if (
          argsForExecute.length === 0 ||
          typeof argsForExecute[argsForExecute.length - 1] !== "object"
        ) {
