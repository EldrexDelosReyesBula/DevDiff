...
export async function configCommand() {
  ...
  try {
    ...
    console.log(pc.bold('Exclude Patterns:'), pc.cyan(config.exclude.join(', ')))
    
    console.log(pc.bold('\nCache Settings:'))
    console.log(`  Enabled: ${config.cache.enabled ? pc.green('yes') : pc.red('no')}`)
    console.log(`  Path:    ${pc.cyan(config.cache.path)}`)

    console.log(pc.bold('\nAI Routing:'))
    console.log(`  Strategy:             ${pc.cyan(config.ai.routing.strategy)}`)
    console.log(`  Complexity Threshold: ${pc.cyan(config.ai.routing.complexityThreshold.toString())}`)
    console.log(`  Local Only:           ${config.ai.routing.localOnly ? pc.green('yes') : pc.red('no')}`)
