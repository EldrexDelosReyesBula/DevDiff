...
export async function auditCommand() {
  ...
  try {
    ...
    const entries = Object.keys(cache).map((hash) => ({
      hash,
      ...cache[hash],
    ...
    console.log(pc.green(`✅ Found ${entries.length} audited AI calls in cache:\n`))
    ...
    console.log(pc.yellow('ℹ️ No audit log file found (or it is empty). Run some generation commands first.'))
  }
}
