...
export async function initCommand(options: { force?: boolean }) {
  ...
  try {
    ...
    console.error(pc.red('Error: Not a Git repository. Please initialize git first (git init).'))
    process.exit(1)
  }
  ...
  if (configExists && !options.force) {
    console.log(pc.yellow(`ℹ️ Configuration file already exists at ${configPath}. Use --force to overwrite.`))
  } else {
  ...
  console.log('Every time you run `git commit`, DevDiff will automatically prepare an AI-generated explanation.')
}
