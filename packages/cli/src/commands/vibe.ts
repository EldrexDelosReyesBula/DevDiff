...
export async function vibeStatusCommand() {
  ...
  const succeededCalls = guardian.session.aiCalls.filter((c: any) => c.success).length;
  const recoveredCalls = guardian.session.aiCalls.filter((c: any) => c.success && guardian.session.failures.some((f: any) => f.checkpointId === c.checkpointId)).length;
  ...
  console.log(`├── AI calls: ${succeededCalls} succeeded, ${failedCalls} failed, ${recoveredCalls} recovered`);
