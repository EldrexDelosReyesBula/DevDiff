...
async function getStagedFiles(): Promise<Array<{ path: string; status: string }>> {
  try {
...
function hashFiles(files: Array<{ path: string; status: string }>): string {
  return files.map(f => `${f.path}:${f.status}`).sort().join(",");
}
...
async function runPrerequisiteChecks() {
  ...
  try {
    ...
    results.push({ name: "ollama", passed: false, message: "Ollama not running" });
  }
  
  // Check cloud AI
  const hasCloudKey = process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY || process.env.GEMINI_API_KEY;
  results.push({ 
    name: "cloud", 
    passed: !!hasCloudKey, 
