...
export async function watchCommand() {
  ...
  try {
    ...
    console.error(pc.red('Error: Not a Git repository. Cannot watch for changes.'))
    process.exit(1)
  }
  ...
  const checkChanges = () => {
    try {
      const diffText = execSync('git diff --cached', { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim()
      if (diffText === lastDiff) return
      
      lastDiff = diffText

      if (!diffText) {
        console.log(pc.gray(`[${new Date().toLocaleTimeString()}] No changes currently staged.`))
        return
      }
      ...
      console.log(pc.gray('Run `devdiff generate` to get full AI explanations.'))
    } catch (err: any) {
