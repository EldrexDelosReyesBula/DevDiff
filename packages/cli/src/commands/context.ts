...
export async function contextCommand(action: ContextAction): Promise<void> {
  ...
  switch (action) {
    ...
    default:
      ...
      console.log("  devdiff context generate  — auto-generate .devdiff/context.md");
      console.log("  devdiff context show      — print the current project context");
      console.log("  devdiff context validate  — check context file for secrets");
...
async function handleGenerate(repoPath: string): Promise<void> {
  ...
  try {
    const { filePath, context, hadSecrets } = await generateContextFile(repoPath);
    ...
    if (context.purpose) {
      const excerpt = context.purpose.length > 80
        ? context.purpose.substring(0, 80) + "..."
        : context.purpose;
      console.log(`   ${pc.bold("Purpose:")} ${excerpt}`);
    }
    if (context.techStack.length > 0) {
      console.log(`   ${pc.bold("Tech stack:")} ${context.techStack.slice(0, 4).join(", ")}`);
    }
    if (context.architecture.length > 0) {
      console.log(`   ${pc.bold("Directories:")} ${context.architecture.length} mapped`);
    }

    if (hadSecrets) {
      console.log("");
      console.log(pc.yellow("⚠️  Secrets were detected and redacted from the context."));
      console.log(pc.yellow("   Review .devdiff/context.md before sharing."));
    }

    console.log("");
    console.log(pc.dim("   💡 Edit .devdiff/context.md to add domain knowledge,"));
    console.log(pc.dim("      naming conventions, or architecture notes."));
    console.log(pc.dim("      Run: devdiff context edit"));
    console.log("");
    console.log(pc.dim("   Context is now injected automatically into every devdiff generate."));
  } catch (error: any) {
...
async function handleShow(repoPath: string): Promise<void> {
  try {
    ...
    if (!loaded) {
      ...
      console.log("   This will auto-scan your project and create .devdiff/context.md");
      return;
    }

    const sourceLabel = loaded.source === "file"
    ...
    console.log(pc.cyan("-----------------------"));
    console.log("");
    console.log(pc.dim(`Character count: ${loaded.raw.length} (~${Math.ceil(loaded.raw.length / 4)} tokens)`));
  } catch (error: any) {
...
async function handleValidate(repoPath: string): Promise<void> {
  ...
  console.log(`   Size: ${report.charCount} characters (~${Math.ceil(report.charCount / 4)} tokens)`);

  if (report.overBudget) {
    console.log(
      pc.yellow(`   ⚠️  Over token budget (${report.charCount} chars > 2000 limit — will be trimmed automatically)`)
    );
  } else {
  ...
  if (report.secrets.length === 0) {
    ...
    for (const secret of report.secrets) {
      const icon = secret.severity === "critical" ? "🔴" : secret.severity === "high" ? "🟠" : "🟡";
      console.log(`   ${icon} [${secret.severity.toUpperCase()}] ${secret.name}`);
    }
    console.log("");
    console.log(pc.yellow("   DevDiff will automatically redact these before sending to AI."));
    console.log(pc.yellow("   But you should remove them from your context file manually."));
    console.log("");
