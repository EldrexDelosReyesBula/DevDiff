  ...
  switch (subcommand) {
    case "list": {
      ...
      schedules.forEach((s) => {
        ...
        console.log(
          `   Cron: ${pc.white(s.cron)} | Action: ${pc.white(s.action)}`,
        );
    ...
    case "enable": {
      if (!options.id) {
        console.log(
          `${pc.red("[lucide:alert-circle]")} Please provide schedule ID using --id`,
        );
        return;
      }
      scheduler.enableSchedule(options.id, true);
      console.log(
        `${pc.green("[lucide:check-circle]")} Schedule ${pc.cyan(options.id)} enabled.`,
      );
      break;
    }
    ...
    case "disable": {
      ...
      console.log(
        `${pc.gray("[lucide:pause]")} Schedule ${pc.cyan(options.id)} disabled.`,
      );
      break;
    }
    ...
    default: {
      console.log(
        `${pc.red("[lucide:alert-circle]")} Unknown schedule subcommand: ${subcommand}`,
      );
