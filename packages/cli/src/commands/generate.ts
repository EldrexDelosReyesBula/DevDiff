...
export async function generateCommand(options: GenerateCmdOptions) {
  ...
  try {
    if (options.range) {
      ...
      diffText = execSync(`git diff ${options.range}`, { stdio: ['ignore', 'pipe', 'ignore'] }).toString()
    } else if (options.commitMsgFile) {
      // Hook mode: analyze staged changes
      diffText = execSync('git diff --cached', { stdio: ['ignore', 'pipe', 'ignore'] }).toString()
    } else {
      // Default: check staged changes first, then fall back to unstaged changes
      diffText = execSync('git diff --cached', { stdio: ['ignore', 'pipe', 'ignore'] }).toString()
      if (!diffText.trim()) {
        diffText = execSync('git diff', { stdio: ['ignore', 'pipe', 'ignore'] }).toString()
      }
    }
  } catch (error) {
    console.error(pc.red('Error: Failed to execute Git commands. Ensure you are in a Git repository.'))
    process.exit(1)
  }
