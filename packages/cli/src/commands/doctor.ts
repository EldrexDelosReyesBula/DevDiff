...
export async function doctorCommand(options: { fix?: boolean }) {
  ...
  try {
    ...
    if (!pkg.type) {
      console.log(pc.yellow("⚠️ Missing \"type\": \"module\" in package.json. This can cause ESM import warnings."));
      ...
      if (hasESMImports || options.fix) {
        if (options.fix) {
          ...
          console.log(pc.blue("💡 Run \"devdiff doctor --fix\" to automatically resolve this issue."));
        }
      }
    } else if (pkg.type === "module") {
      console.log(pc.green("✅ package.json has \"type\": \"module\"."));
    } else {
      console.log(pc.yellow(`⚠️ package.json has "type" set to "${pkg.type}". DevDiff recommends ES Modules.`));
    }
  } catch (err: any) {
