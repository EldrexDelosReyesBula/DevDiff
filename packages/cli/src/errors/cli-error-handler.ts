...
function findSimilarCommands(cmd: string): string[] {
  ...
  return COMMAND_REGISTRY.map((c) => c.name).filter(
...
export class CLIErrorHandler {
  static handle(error: Error, command?: string): never {
    ...
    if (
      ...
      console.log("");
      console.log(
        "   DevDiff analyzes files that have been staged with git add.",
      );
      console.log("");
    ...
    if (
      ...
      (error.message.includes("ECONNREFUSED") ||
        error.message.includes("NotAvailable"))
    ) {
      console.log("");
    ...
    if (
      ...
      const modelName =
        error.message.match(/model ['"]([^'"]+)['"]/)?.[1] || "unknown";
    ...
    if (
      ...
      error.message.includes("not a command")
    ) {
    ...
    console.log(
      "   Report issue: https://github.com/eldrex/devdiff/issues/new",
    );
    console.log("");
    console.log("   Include:");
