  ...
  explain(params: {
    ...
    if (/def\s+\w+\s*\(.*?\)\s*->/i.test(params.code)) {
      base.sections.push({
        ...
        content: "Uses Python type hints (`->`, `: int`, `: str`) to document input and return types.",
    ...
    if (/\[\s*x\s+for\s+x\s+in/i.test(params.code)) {
      base.sections.push({
        ...
        content: "Uses pythonic list comprehension syntax for concise collection transformation.",
        importance: "important",
      });
    }
