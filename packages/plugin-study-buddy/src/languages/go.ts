  ...
  explain(params: {
    ...
    if (/go\s+\w+\(|chan\s+/i.test(params.code)) {
      base.sections.push({
        ...
        content: "Uses Go lightweight goroutines (`go fn()`) and channels (`chan`) for CSP concurrency.",
        importance: "essential",
      });
      base.relatedConcepts.push("Goroutines", "Channels & Select", "CSP Concurrency");
    }
