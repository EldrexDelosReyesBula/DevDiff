...
export const CostTrackerPlugin: DevDiffPlugin = {
  ...
  version: "1.0.0",
  description: "Tracks cumulative token usage and estimates API costs across local/cloud AI calls.",
  author: {
  ...
  async activate(context: PluginContext) {
    ...
    context.logger.info(`Cost Tracker active. Historical tokens used: ${totalTokens}`);
  },

  hooks: {
    async onAIComplete(result: AIResult) {
      const tokens = result.tokensUsed || 0;
      console.log(`[Cost Tracker] Model ${result.model} consumed ${tokens} tokens.`);
    },
  },
};
