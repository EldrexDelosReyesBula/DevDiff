...
try {
  ...
  if (findings.length > 0) {
    ...
    for (const f of findings) {
      console.log(`[${f.severity.toUpperCase()}] ${f.file}: ${f.name} - "${f.content}"`);
    }
  }
  ...
  if (process.env.GITHUB_OUTPUT) {
    ...
    fs.appendFileSync(process.env.GITHUB_OUTPUT, `report-url=https://github.com/${process.env.GITHUB_REPOSITORY}/actions/runs/${process.env.GITHUB_RUN_ID}\n`);
  }
} catch (err) {
